...
import { createEditGarageBranch } from '../../../../hooks/GarageApi';
import { TxtInput } from '../../../../components/TxtInput';
import { Messages } from '../../../../components/Messages';


export const CreateEditGarageBranch = ({navigation, route}) => {
    ...
    const createEditAttempt = createEditGarageBranch(formik.setErrors, formik.values);
            ...
            <View className="flex-1 items-center justify-center p-8">
                <View className="w-full p-8 max-w-sm">
                    <Text className="text-lg font-extrabold text-gray-200 text-center mb-2">
                        { formik.values.id == '' ? 'Add new Branch to this Garage' : 'Update Branch' }
                    </Text>

                    <FormikInput valueName="email" formik={formik} placeholder="Garage Email:" label={formik.values.id == '' ? null : 'Garage Email: '}/>
                    <FormikInput valueName="telephone" formik={formik} placeholder="Garage Telephone:" label={formik.values.id == '' ? null : 'Garage Telephone: '}/>

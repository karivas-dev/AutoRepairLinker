    ...
    if (data?.links?.next != null) {
      return (
        <View className="mt-2">
          <PrimaryButton
            message={"load more"}
            onPress={() => navigation.setParams({ page: page + 1 })}
          ></PrimaryButton>
        </View>
      );
    }
      ...
      <View className="flex-1 items-center justify-center">
        <View className="w-full max-w-sm">
          <View className="flex flex-row justify-between">
            <Text className="font-bold mb-6 text-gray-200 mt-5 text-2xl">
              Replacements
            </Text>
            <View className="justify-end mt-5 mb-6">
              <PrimaryButton
                onPress={() => (
                  navigation.navigate("CreateEditForm", {
                    id: "",
                    name: "",
                    description: "",
                    brand_id: "",
        ...
        <View className="flex-1 w-full max-w-sm">
          {isLoading || isFetching ? (
            ...
            error.response.data?.message ? (
              ...
              <Messages
                message={`Here was a problem processing Replacements : ${error.message}`}
                level={"error"}
              />
            )
            ...
            <Messages
              message={"No data of Replacements in our records ..."}
              level={"info"}
            />
          )}
        </View>

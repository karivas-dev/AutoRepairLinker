...
export const ReplacementList = ({navigation, route}) => {

    //flash message
    const {level, flashMessage, page} = route.params;

    const {data, isLoading, isError, isFetching, error, replacements} = getReplacements(page);
    ...
    const handleSearch = (text) => {
        ...
        if(text.trim().length !== 0 ){
            let filteredData = replacements.filter((replacement) => {
                ...
                return (lowername.includes(textSearch.trim()) || lowerDescription.includes(textSearch.trim()));
            });
            ...
            <Card>
                <View className="flex flex-row" >
                    ...
                    <View className="grow mt-4">
                        <View className="ml-8" >
                            <Text className="text-gray-200 text-md font-bold ">{ replacement.name }</Text>
                            <Text className="text-gray-200 text-md ">{ shortDescripton(replacement.description) }</Text>
                        </View>
                    </View>
                    <View className="py-4">
                        <Pressable onPress={() => (navigation.navigate('DetailReplacement',{ id: replacement.id }), setSearch(''))}>
                ...
                <View className="w-full max-w-sm">
                    <View className="flex flex-row justify-between">
                        ...
                        <View className="justify-end mt-5 mb-6">
                            <PrimaryButton onPress={() => (setSearch(''))} message="+ mmm"/>
                        </View>
                    </View>
                    {
